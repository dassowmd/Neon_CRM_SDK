...
from neon_crm.types import CreateIndividualAccountPayload, CreateCompanyAccountPayload, CompleteAccountPayload
...
def write_regression_client():
    ...
    if not org_id or not api_key:
        pytest.skip("Write tests require NEON_ORG_ID and NEON_API_KEY environment variables")
        
    if allow_write_tests != "true":
        pytest.skip("Write tests require NEON_ALLOW_WRITE_TESTS=true environment variable")
        
    if environment == "production":
        pytest.fail("Write tests should NEVER be run against production! Set NEON_ENVIRONMENT=trial")
...
class TestAccountWriteOperations:
    ...
    def test_create_company_account_minimal(self, write_regression_client):
        ...
        try:
            ...
            # Verify the account was created
            created_account = write_regression_client.accounts.get(account_id)
            assert created_account["companyName"] == f"Test Company {timestamp}" or "companyName" in created_account
    ...
    def test_update_account(self, write_regression_client):
        ...
        try:
            ...
            updated_result = write_regression_client.accounts.update(account_id, update_data)
    ...
    def test_patch_account(self, write_regression_client):
        ...
        try:
            ...
            patched_result = write_regression_client.accounts.patch(account_id, patch_data)
...
class TestHouseholdWriteOperations:
    ...
    def test_household_member_management(self, write_regression_client):
        ...
        try:
            ...
            household_id = household_result.get("householdId") or household_result.get("id")
            print(f"✓ Created test household: {household_id}")
            
            # Add member to household
            if household_id:
                add_result = write_regression_client.households.add_member(household_id, account2_id)
                print(f"✓ Added member {account2_id} to household {household_id}")
                
                # Remove member from household
                remove_result = write_regression_client.households.remove_member(household_id, account2_id)
...
class TestWebhookWriteOperations:
    ...
    def test_webhook_lifecycle(self, write_regression_client):
        ...
        try:
            ...
            if webhook_id:
                # Update the webhook
                update_result = write_regression_client.webhooks.update_webhook(
                ...
                # Test the webhook
                try:
                    test_result = write_regression_client.webhooks.test_webhook(webhook_id)
    ...
    def test_cancel_recurring_donation(self, write_regression_client):
        ...
        try:
            ...
            if recurring_donations:
                donation_id = recurring_donations[0].get("id") or recurring_donations[0].get("donationId")
                
                if donation_id:
                    print(f"⚠ Would cancel recurring donation {donation_id} (test skipped for safety)")
...
def require_explicit_confirmation():
    ...
    def decorator(test_func):
        def wrapper(*args, **kwargs):
            ...
            if confirm != "true":
                pytest.skip(f"Dangerous test {test_func.__name__} requires NEON_CONFIRM_DANGEROUS_TESTS=true")
