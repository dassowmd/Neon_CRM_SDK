...
_current_permissions: ContextVar[Optional[UserPermissions]] = ContextVar('current_permissions', default=None)


class PermissionError(NeonAPIError):
    """Raised when a user lacks required permissions."""
    
    def __init__(self, message: str, resource: ResourceType, permission: Permission, user_id: str = None):
        ...
        def wrapper(*args: "P.args", **kwargs: "P.kwargs") -> "T":
            ...
            if user_permissions is None:
                raise PermissionError(
                    f"Authentication required to access {resource.value}",
                    resource=resource,
                    permission=permissions[0]  # Use first permission for error reporting
                )
...
class PermissionChecker:
    ...
    @staticmethod
    def ensure_permission(resource: ResourceType, permission: Permission, custom_message: str = None):
        ...
        if permissions is None:
            message = custom_message or f"Authentication required to access {resource.value}"
    ...
    @staticmethod
    def ensure_any_permission(resource: ResourceType, permissions_list: list[Permission], custom_message: str = None):
        ...
        if permissions is None:
            message = custom_message or f"Authentication required to access {resource.value}"
            raise PermissionError(message, resource=resource, permission=permissions_list[0])
        
        if not permissions.has_any_permission(resource, permissions_list):
            perm_names = [p.value for p in permissions_list]
            message = (custom_message or 
                      f"User '{permissions.user_id}' lacks required permissions for '{resource.value}'. "
                      f"Needs one of: {', '.join(perm_names)}")
            raise PermissionError(
                message,
                resource=resource,
