...
class PermissionConfig:
        ...
        for user_id, user_config in users_data.items():
            try:
                ...
                if "permissions" in user_config:
                    for resource_name, permissions in user_config[
                        "permissions"
                    ].items():
                        resource = ResourceType(resource_name)
                        perms = {Permission(p) for p in permissions}
                ...
                user_permissions = create_user_permissions(
                    user_id, role, custom_overrides
                )
    ...
    def add_user(
        ...
        user_id: str,
        role: Role,
        custom_permissions: Dict[ResourceType, Set[Permission]] = None,
    ) -> UserPermissions:
    ...
    def update_role_permissions(
        self, role: Role, resource: ResourceType, permissions: Set[Permission]
    ):
    ...
    def list_users_with_permission(
        self, resource: ResourceType, permission: Permission
    ) -> list[str]:
...
default_permission_matrix.add_user(
    "volunteer_coordinator_user", Role.VOLUNTEER_COORDINATOR
)
